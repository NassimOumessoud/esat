...
def analyse(result, workbook, sheet, sheet_2, exc_index, name, weight, x_result, path):
    """
    ...
    collapses, rises = peak_valley_change(valleys[0], valleys[1], peaks[0], peaks[1])
...
def peaks_and_valleys(x, y):  # peak and valley detection
    ...
    for i in range(len(y) - 2):
        if y[i + 1] >= y[i] and y[i + 1] >= y[i + 2]:
            ...
            y[i + 1] <= y[i] and y[i + 1] <= y[i + 2] and dal_waarden[-1] <= y[i + 1]
        ):  # define valleys and make sure that they are properly spaced
            dal = y[i + 1]
            x_dal = x[i + 1]
            for r in range(i, len(y) - 2):
...
def plot(
    data, trend, x_trend, data_1, data_2, data_3, data_4, data_5, x_data, name, path
):
    """
